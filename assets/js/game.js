		...
		if (object == "bullet") {
			...
			for (var i = 0; i < size; i++) {
				...
				enemy.init(0,0, imageRepository.enemy.width,
									 imageRepository.enemy.height+1);
				pool[i] = enemy;
			}
		}
...
function Ship() {
	...
	this.move = function() {
		...
		if (KEY_STATUS.left || KEY_STATUS.right) {
			...
			if (KEY_STATUS.left) {
				this.x -= this.speed
				if (this.x <= -8) // Kep player within the screen
					this.x = -8;
...
function Game() {
	...
	this.spawnWave = function(level) {
		//console.log(level)
		var lvl = level;
		var color = imageRepository.enemy.srcArray[lvl];
		//imageRepository.enemy.src = color;
...
function SoundPool(maxSize) {
	...
	this.init = function(object) {
		if (object == "laser") {
			for (var i = 0; i < size; i++) {
				// Initalize the object
				laser = new Audio("assets/sounds/hero-laser.wav");
				laser.volume = .12;
				laser.load();
				pool[i] = laser;
			}
			...
			for (var i = 0; i < size; i++) {
				var explosion = new Audio("assets/sounds/enemy-hurt.wav");
				explosion.volume = .3;
				explosion.load();
				pool[i] = explosion;
			}
